...
// New methods for handling instances

func (s *StampService) CreateStampInstance(instance *models.StampInstance) (*models.StampInstance, error) {
...
func (s *StampService) UpdateStampInstance(instance *models.StampInstance) (*models.StampInstance, error) {
...
func (s *StampService) DeleteStampInstance(id string) error {
	now := time.Now().Format(time.RFC3339)
	_, err := s.db.Exec("UPDATE stamp_instances SET date_deleted = ? WHERE id = ? AND date_deleted IS NULL", now, id)
	return err
}

func (s *StampService) GetStampInstance(id string) (*models.StampInstance, error) {
